...
5. **Validation & Persistence:** Present plan to user. 
   - **ACTION:** Once approved, write the plan to `engineer/templates/implementation_plan.md`.
   - **HALT:** End session. Do not write implementation code.

## [MODE: IMPLEMENT]
0. **Grounding:** Read `AI Context` or `GEMINI.md`.
1. **Load Contract:** Read `engineer/templates/implementation_plan.md`. 
...
7. **Delivery:** `git add` and `git commit` to a feature branch. 
   ...
   1. **Architectural Intent:** Execute `/plan` logic. Draft the `engineer/templates/implementation_plan.md`.
