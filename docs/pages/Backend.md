...
- # Backend (Deep Specification)
	...
	- ## Commands
		...
		- **`backend:review`**: Structured, stack-aware code review against a GitHub, GitLab, or Azure DevOps pull request. Auto-detects platform from PR URL, acquires diff via MCP (falls back to CLI instructions), runs Phase 0.4 to detect the backend stack from changed file extensions (.NET/Java/Go, agent-scoped), self-loads the relevant `common/stacks/*.md` guardrails, and applies the full backend-specific review protocol (DB patterns, resilience, API contracts, security). Hard Guardrail violations are auto-elevated to Critical/High severity. Run from the project root for optimal context loading (AGENTS.md, CLAUDE.md, GEMINI.md). (ref: `backend/commands/backend/review.toml`, `common/skills/pr_review.md`)
